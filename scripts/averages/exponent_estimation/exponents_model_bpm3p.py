...
gamma, beta, rho = 1.2, 1, 1
model = BPM3pProcess(gamma, beta, rho)
...
t_asymp_1 = 8000
t_asymp_2 = 9800
t_asymp_start = (t_asymp_1 - t0) // delta
t_asymp_end = (t_asymp_2 - t0) // delta
...
t_vals = np.arange(delta, max_T + 1, delta)
...
slope_abs, intercept_abs, r_value, p_value, std_err = linregress(
    t_avg_log[t_asymp_start:t_asymp_end], avg_abs_log[t_asymp_start:t_asymp_end])
slope_sq, intercept_sq, r_value, p_value, std_err = linregress(
    t_avg_log[t_asymp_start:t_asymp_end], avg_sq_log[t_asymp_start:t_asymp_end])
...
print(f'Moses exponent (formula): {gamma/rho - 1/2:.4f}')
...
ax_1.axvline(t_avg_log[t_asymp_start - 1], linestyle='--')
ax_1.axvline(t_avg_log[t_asymp_end - 1], linestyle='--')
