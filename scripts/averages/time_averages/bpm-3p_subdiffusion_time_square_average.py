...
avg_t = averager.time_average_as_function_of_t(sample_path, min_T, max_T, delta, time_step, average_type='sq')

slope, intercept, r_value, p_value, std_err = linregress(np.log(t_axis), np.log(avg_t))

fig, ax = plt.subplots(figsize=(8, 5))

plt.loglog(t_axis, avg_t, 'o-', label='Square velocity average - Slope = ' + str(slope))
