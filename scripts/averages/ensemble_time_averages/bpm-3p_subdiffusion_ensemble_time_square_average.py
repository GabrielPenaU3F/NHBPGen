...
ensemble = np.load('../../../test/test_data/test_ensemble_subdif.npz').get('ensemble')
...
ax.loglog(t, avgs_t, '-', label='Square velocity average', color='#000681')
ax.loglog(t[10:], y_regresline[10:], label='Regression line', color='#E71700', linewidth=1.0)

ax.set_title('Square velocity ET-average (subdiffusion)', fontsize=14)
...
fig.tight_layout()
plt.savefig('subdif_noah.eps', format='eps')
plt.show()
