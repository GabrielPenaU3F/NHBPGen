...
#   In this queue, the arrival and service rate both see the cumulative number
#   of arrival as its k
class ArrivalAdaptiveQueue(QueueSystem):
    ...
    def simulate_queue(self, time):
        ...
        while present_time < time:
            ...
            present_time = self.generate_next_transition_time(cumulative_arrivals, present_time)
            # The conditional probability of a birth must be calculated before updating the nº of arrivals,
            # because the system still does not know if it is a birth or a death
            current_lambda = self.arrival_process.intensity_function(cumulative_arrivals, present_time)
            current_mu = self.service_process.intensity_function(cumulative_arrivals, present_time)
