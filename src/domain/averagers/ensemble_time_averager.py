...
class EnsembleTimeAverager:
    ...
    def average_as_function_of_t(self, ensemble, max_T, delta, time_step, average_type='regular'):
        ta_ensemble_t = self.time_average_ensemble_as_function_of_t(ensemble, max_T, delta, time_step, average_type)
    ...
    def time_average_ensemble_as_function_of_t(self, ensemble, max_T, delta, time_step, average_type):
        time_averager = TimeAverager()
        avg_ensemble_t = []
        for i in range(len(ensemble)):
            path = ensemble[i]
            avg_t = time_averager.time_average_as_function_of_t(path, max_T, delta, time_step, average_type)
            avg_ensemble_t.append(avg_t)
            print(f"Time-averaging trajectory n={i+1} ...")
        return np.array(avg_ensemble_t)

    def estimate_moses(self, ensemble, max_T, delta, time_step):
        t = np.arange(delta, max_T + time_step, time_step)
        avgs_t = self.average_as_function_of_t(ensemble, max_T, delta, time_step, average_type='abs')
        slope, intercept, r_value, p_value, std_err = linregress(np.log(t), np.log(avgs_t))
        M = slope + 1/2
        print(f'Moses exponent: M={M}')
        return M

    def estimate_noah(self, ensemble, moses, max_T, delta, time_step):
        t = np.arange(delta, max_T + time_step, time_step)
        sq_avgs_t = self.average_as_function_of_t(ensemble, max_T, delta, time_step, average_type='sq')
